...
<UnOrderedList>
  <ListItem>Uses function instead of a class</ListItem>
  <ListItem>Simplified by reducing template code</ListItem>
...
```jsx title="Example with useState()"
import React from "react";
...
<UnOrderedList>
  <ListItem>Return a stateful value and a function to replace it.</ListItem>
  <ListItem>Takes in an initial state.</ListItem>
  <ListItem>setState takes in the new state and enqueues a re-render.</ListItem>
  <ListItem>Unlike `setState` useState doesn't merge objects.</ListItem>
</UnOrderedList>
...
<UnOrderedList>
  ...
  <ListItem>Runs after the render complete render cycle.</ListItem>
  <ListItem>
    Takes in an initial value and initializes current property of the ref
    object.
  </ListItem>
  ...
  <ListItem>
    Same as `ComponentDidUpdate` and `ComponentWillunmount` lifecycle methods in
    classes.
  </ListItem>
</UnOrderedList>
...
<UnOrderedList>
  <ListItem>Signature identical to `useEffect` hook</ListItem>
...
<UnOrderedList>
  <ListItem>Accepts a context object.</ListItem>
  <ListItem>Returns the current context value for that context.</ListItem>
  <ListItem>
    Component calling the `useContext` is always rerender when the context value
    changes.
  </ListItem>
  <ListItem>
  ...
  <ListItem>
    `MyContext.Provider` is still needed in the tree to provide the context
    value.
  </ListItem>
</UnOrderedList>
...
<UnOrderedList>
  <ListItem>Is an alternative to `useState`</ListItem>
  <ListItem>Accepts a reducer of type `(state, action) => newState`</ListItem>
  <ListItem>Returns the current `state` paired with `dispatch`</ListItem>
...
<UnOrderedList>
  ...
  <ListItem>
    Like `useCallback`, it takes in a function as an argument and an array of
    dependencies.
  </ListItem>
  <ListItem>
...
<UnOrderedList>
  ...
  <ListItem> Should be used with `forwardRef`</ListItem>
</UnOrderedList>
