...
import { CardWithCallback } from "./components/CardWithCallback";
...
const IntervalRef = ({ setClean, setCount, count }) => {
  ...
  useEffect(() => {
    // Starts setInterval and set interval id
    if (count === 0) {
      intervalIdRef.current = setInterval(() => setCount(count + 1), delay);
    }
  }, [count]);
...
```jsx title="CardWithoutCallback" 7,8
export const CardWithoutCallback = () => {
  import React, { useState } from "react";
...
```jsx title="CardWithCallback" 1:13
...
export const CardWithCallback = () => {
  ...
  return <Card text='👽' title='Callback Card' fn={setter} />;
};
```
...
```jsx title="Example with useDebugValue"
